...
def index():
    ...
    for file in files:
        result += f"<a href='/music/{file}' download='{file}'>{file} {convert_size(os.path.getsize(f'/music/{file}'))}</a><br><audio controls><source src='/music/{file}' type='audio/mp3'></audio><br><br>"
    if result:
        return result
    else:
        return "nothing"
