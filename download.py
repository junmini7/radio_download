...
print(real_url)
subprocess.Popen(f'ffmpeg -i "{real_url}" -vn -acodec libmp3lame -t 15 -metadata title="Every_music_{today_date}" -metadata date="{today_date}" -metadata album="KBS" -metadata track="{today_date}" ./data/everymusic_{today_date}.mp3')
