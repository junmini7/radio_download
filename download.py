...
def index():
    ...
    <button class='btn btn-danger' onclick='delete_file("{file}");'>삭제</button></div></div><div class="col-12 col-md-5 col-xl-3 centering" style="margin-bottom:10px"><div class="row">
    ...
    for id in record_channel_ids:
        ...
        for program_schedule in schedules:
            if (
                    ...
                    program_schedule["start"] - td(seconds=3)
                    <= dt.now()
